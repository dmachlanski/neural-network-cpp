...
public:
	TwoLayerNN();
	void InitializeModel(string inputDataPath, string outputDataPath, int hiddenUnits, bool normalizeBeforeSplit, bool isOutputLinear);
	void LoadModel(string path);
	void SaveModel(string path, bool saveBestWeights);
	void Train(double learningRate, double momentum, int batchSize, int epochs, int printOn);
...
private:
	...
	VectorXd b1, b2, gradB1, gradB2, muX, sigmaX, muY, sigmaY, J, JEpoch, Valid, ValidEpoch, bestB1, bestB2, db1, db2;
};